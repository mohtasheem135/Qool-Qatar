...
let Stepper = ({
  ...
  return (
    <div className="stepper-wrapper">
      <div
        style={{ display: isVertical ? "flex" : "block" }}
...
const App1 = (props) => {
  ...
  const onChangeClick = (e, time) => {
    ...
    let appendDataToSelectedPackage = JSON.parse(
      localStorage.getItem("selectedPackageData")
    );
    appendDataToSelectedPackage["timeSlot"] = { ...time };

    localStorage.setItem(
      "selectedPackageData",
      JSON.stringify(appendDataToSelectedPackage)
    );
  };
  ...
  const stepperContent = [
          ...
          <div className="package-block">
            <div className="package-sub">
              ...
              <div style={{ display: "flex", flexDirection: "column" }}>
                ...
                <p className="txt3">
                  <span>QAR {packageData?.price}/</span>person
                </p>
              </div>
            </div>
            <Button className="tour-btn">
              <img
                ...
                alt="calendar"
              />
              {dateFormat(packageData?.eventStartDate, "DDD mmm , yyyy")}
            </Button>
          </div>
          <div>
                ...
                return (
                  <FormCheck className="select-tab">
                    <FormCheckInput
                      ...
                      checked={
                        checkedData?.checked && checkedData.id === time._id
                      }
                      onChange={(e) => onChangeClick(e, time)}
                    ></FormCheckInput>
                    <FormCheckLabel>
                      <p className="time">
                        {time.startingTime} - {time.endTime}
                      </p>
                      <p
                        className="price"
                      ...
                      >
                        ...
                        <span>
                          {time.available}/{time.total} available
                        </span>
                      </p>
                    </FormCheckLabel>
                  </FormCheck>
    ...
    },
    {
          ...
          <div className="booking-form book-package">
            <Form>
              ...
              <FormGroup className="guest-box">
                ...
                <NumericInput
                  mobile
                  value={initialState.guest}
                  name="guest"
                  onChange={handleGuest}
                  className="form-control numeric-box"
                  min={1}
                  max={selectedTimeSlot?.available}
                />
              </FormGroup>
              <p className="available-txt">
                <span>{selectedTimeSlot?.available}</span> /{" "}
                {selectedTimeSlot?.total} Available{" "}
              </p>
            </Form>
          </div>
        </div>
      ...
      isComplete:
        initialState?.guest && initialState?.name && initialState?.mobileNumber,
    },
    {
      label: "Checkout",
      content: (
        <div>
          ...
          <div className="fee-block">
            ...
            <div className="fees1 list1">
              ...
              <p>
                QAR{" "}
                {packageData.price * initialState.guest +
                  packageData?.tax * initialState.guest}
              </p>
            </div>
          </div>
          <div className="tell-us">
          ...
          <div className="details1">
            ...
            <div className="details12">
              <div>
                <p className="name">
                  {initialState?.name + initialState?.lastName}
                </p>
